...
class TestMetricsEvaluator(unittest.TestCase):
    ...
    def test_evaluate_single(self):
        ...
        result = self.evaluator.evaluate_single(
            before_code, after_code, predicted_code, "test_1"
        )
    ...
    def test_evaluate_batch(self):
        ...
        results = self.evaluator.evaluate_batch(
            before_codes, after_codes, predicted_codes
        )
    ...
    def test_aggregate_results(self):
        ...
        self.assertEqual(
            aggregated["diff_edit_similarity"]["mean_diff_edit_similarity"], 1.0
        )
...
def run_simple_examples():
    ...
    result = evaluator.evaluate_single(
        before_code, after_code, predicted_code, "perfect_fix"
    )
    ...
    result = evaluator.evaluate_single(
        before_code, after_code, wrong_predicted, "wrong_fix"
    )
    print(f"Similarity: {result.metrics['diff_edit_similarity'].score:.3f}")
    print(f"Metadata: {result.metrics['diff_edit_similarity'].metadata}\n")
    ...
    print(
        f"Mean similarity: {aggregated['diff_edit_similarity']['mean_diff_edit_similarity']:.3f}"
    )
    print(
        f"Std similarity: {aggregated['diff_edit_similarity']['std_diff_edit_similarity']:.3f}"
    )
